...
void Reese::retemper(){
	Oscillator::retemper();
	for( int n=0; n < 128; ++n ){
		notes[n].phase  *= notes[n].freq * detune;
		notes[n].phase2 *= notes[n].freq / detune;
	}
}
...
SubSineTbl::SubSineTbl()
{
	...
	{
		table[t] =  (float) 0.5 * sin(t * f);       // fundamental
		table[t] += (float) 0.3 * sin(t * 2.0 * f); //    +8va
		table[t] += (float) 0.2 * sin(t * 0.5 * f); //    -8va
	}
}

SubSineOsc::SubSineOsc() : 	curNote(127)
{
	wt = new SubSineTbl();
}
...
void SubSineOsc::noteOn(int k, int v)
{
	...
	while( note.freq < testVal1 )
		note.freq *= 2;
	while( note.freq > testVal2 )
		note.freq /= 2;
