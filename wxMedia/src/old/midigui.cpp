...
void KeyboardPanel::OnPaint(wxPaintEvent &WXUNUSED(event)){
	...
	n = i % 12;
	o = i / 12;
	dc.SetBrush(wxColor(0x00, 0x66, 0xff));
	dc.SetPen(  wxColor(0x00, 0x66, 0xff));
	dc.DrawRectangle( 0, height - keyspacing,
			keyspacing * 4 + keysX + keyspacing / 2 + ((o-2) * 14 * keyspacing) + noteval[n] * keyspacing - keyspacing / 2,
			keyspacing * 2 );				
	...
	dc.SetBrush(wxColor(0x00, 0x66, 0xff));
	dc.SetPen(  wxColor(0x00, 0x66, 0xff));
	dc.DrawRectangle( keyspacing * 4 + keysX + keyspacing / 2 + ((o-2) * 14 * keyspacing) + noteval[n] * keyspacing - keyspacing / 2,
			height - keyspacing,
			width,
			keyspacing * 2 );				
